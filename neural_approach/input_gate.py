...
def validate_speech_signal(
	...
	if waveform.size == 0 or sample_rate <= 0:
		logger.warning("Neural speech gate failed: empty waveform or invalid sample rate")
	...
	if not is_valid:
		logger.warning(
			"Neural speech gate rejected audio: duration=%.3f rms_dbfs=%.2f voiced_ratio=%.3f clipping_ratio=%.3f",
			duration_sec,
			rms_dbfs,
			voiced_ratio,
			clipping_ratio,
		...
		logger.info("Neural speech gate passed: duration=%.3f", duration_sec)
